...
function FormCheck(props) {
    ...
    const getFood = async () => {
        try {
          ...
          const array = data.docs.map((doc) => ({id:doc.id,label:doc.data().foodName,hunger:doc.data().hungerlevel,weight:doc.data().userweight}));
          console.log(array);
          setFood(array);
        //   console.log(food);
        } catch (error) {
    ...
    return ( 
        <div className='form'>
            <div className='form-body'>
                ...
                <div className='entry flex flex-row w-screen items-center justify-center'>
                    <div className={`portionbtn ${fullness===1 ? 'btn-selected' : ''}`} onClick={() => setFullness(1)}>1</div>
                    <div className={`portionbtn ${fullness===2 ? 'btn-selected' : ''}`} onClick={() => setFullness(2)}>2</div>
                    <div className={`portionbtn ${fullness===3 ? 'btn-selected' : ''}`} onClick={() => setFullness(3)}>3</div>
                    <div className={`portionbtn ${fullness===4 ? 'btn-selected' : ''}`} onClick={() => setFullness(4)}>4</div>
                    <div className={`portionbtn ${fullness===5 ? 'btn-selected' : ''}`} onClick={() => setFullness(5)}>5</div>
                </div>
            <Submit hunger={hunger} store={props.store} fullness='2' name={name} foodsRef={foodsRef}/>
...
const Submit = ({hunger,store,fullness,name,setTest,test, foodsRef,getFood}) => {
    let obj = {
        foodName:name,
